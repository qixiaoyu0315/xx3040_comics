...
def view_all_comics():
    ...
    for comics in comics_x3040_db.find():
        comics_list.append({'uid': comics['uid'], 'tittle': comics['tittle'], 'look_url': f"/comics/{comics['uid']}/1",
                            'len': len(comics['page_item'])})
    return comics_list
...
# 更新单个漫画
def update_comics_one(uid):
    comics = comics_x3040_db.find_one({'uid': uid})
    if comics:
        comics_update = get_comics_info(comics['html_url'])
        # 如果网页的页数 大于 已有页数 则更新
        if len(comics_update['page_item']) > len(comics['page_item']):
            new_page_item = comics['page_item']
            index_list = [index for index in new_page_item.keys()]

            for index, page_info in comics_update['page_item'].items():
                if index not in index_list:
                    new_page_item.update({index: page_info})
            comics_x3040_db.update_one({'_id': comics['_id']}, {'$set': {'page_item': comics_update['page_item']}})


# 更新所有漫画
def update_all_comics():
    for comics in comics_x3040_db.find({}):
        update_comics_one(comics['uid'])
